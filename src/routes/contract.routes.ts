...
import {Contract, AuditLog, Agent, TheParties} from '../models';
...
router.get('/getPriorInfo/agent/:agentId/property/:propertyId',authGuard, async (req: AuthReq, res: Response): Promise<void> => {
  const id = req.agentId;
  if(req.params.agentId!=id){
    res.status(401).json({msg:'other agent'})
    return;
  }
    const agentInfo = await Agent.findById(id)
        .populate('manageProperty')
        .lean();
  console.log(agentInfo);
  const info  =
      await Agent.findById(id).select('name address compName phoneNum brokerNo').
      populate({
          path: 'manageProperty',
